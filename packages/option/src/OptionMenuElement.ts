 ...
 *
 * @description
 * The `m3e-option-menu` component renders a scrollable container for displaying selectable options
 ...
 *
 * @cssprop --m3e-option-menu-container-shape - Controls the corner radius of the menu container.
 * @cssprop --m3e-option-menu-container-min-width - Minimum width of the menu container.
 * @cssprop --m3e-option-menu-container-max-width - Maximum width of the menu container.
 * @cssprop --m3e-option-menu-container-max-height - Maximum height of the menu container.
 * @cssprop --m3e-option-menu-container-padding-block - Vertical padding inside the menu container.
 * @cssprop --m3e-option-menu-container-color - Background color of the menu container.
 * @cssprop --m3e-option-menu-container-elevation - Box shadow elevation of the menu container.
 * @cssprop --m3e-option-menu-divider-spacing - Vertical spacing around slotted `m3e-divider` elements.
 */
@customElement("m3e-option-menu")
export class M3eOptionMenuElement extends Role(LitElement, "listbox") {
  ...
  static override styles: CSSResultGroup = css`
    :host {
      ...
      border-radius: var(--m3e-option-menu-container-shape, ${DesignToken.shape.corner.extraSmall});
      min-width: var(--m3e-option-menu-container-min-width, 7rem);
      max-width: var(--m3e-option-menu-container-max-width, 17.5rem);
      max-height: var(--m3e-option-menu-container-max-height, 17.5rem);
      padding-block: var(--m3e-option-menu-container-padding-block, 0.5rem);
      background-color: var(--m3e-option-menu-container-color, ${DesignToken.color.surfaceContainer});
      box-shadow: var(--m3e-option-menu-container-elevation, ${DesignToken.elevation.level3});
      opacity: 0;
      display: none;
    }
    :host(:not(.-no-animate)) {
    ...
    ::slotted(m3e-divider) {
      margin-block: var(--m3e-option-menu-divider-spacing, 0.5rem);
    }
    @starting-style {
      :host(:popover-open) {
        transform: scaleY(0.8);
  ...
  #handleDocumentClick(e: MouseEvent): void {
    if (!e.composedPath().some((x) => x instanceof M3eOptionMenuElement || x === this.#trigger || x === this.#anchor)) {
      this.hide();
    }
  }
}

interface M3eOptionMenuElementEventMap extends HTMLElementEventMap {
  beforetoggle: ToggleEvent;
  toggle: ToggleEvent;
}

export interface M3eOptionMenuElement {
  addEventListener<K extends keyof M3eOptionMenuElementEventMap>(
    type: K,
    listener: (this: M3eOptionMenuElement, ev: M3eOptionMenuElementEventMap[K]) => void,
  ...
  removeEventListener<K extends keyof M3eOptionMenuElementEventMap>(
    type: K,
    listener: (this: M3eOptionMenuElement, ev: M3eOptionMenuElementEventMap[K]) => void,
...
declare global {
  interface HTMLElementTagNameMap {
    "m3e-option-menu": M3eOptionMenuElement;
  }
}
