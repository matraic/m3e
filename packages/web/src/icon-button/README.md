...
All appearance variants (`standard`, `outlined`, `filled`, `tonal`) support the following properties (replace `[variant]` with the variant name). Omitting the `[variant]-` prefix overrides the value for all variants.
